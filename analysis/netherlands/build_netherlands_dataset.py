from pathlib import Path
import json

import numpy as np
import pandas as pd

from loonheffing_2026 import compute_loonheffing_2026


ROOT_DIR = Path(__file__).resolve().parents[2]

PARAMETERS_PATH = (
    ROOT_DIR
    / "docs"
    / "data"
    / "netherlands"
    / "netherlands_parameters_2026.json"
)

OUTPUT_PATH = (
    ROOT_DIR
    / "docs"
    / "data"
    / "netherlands"
    / "netherlands_labour_cost_grid_2026.csv"
)


def load_parameters() -> dict:
    with PARAMETERS_PATH.open("r", encoding="utf-8") as file:
        return json.load(file)


def round_money(value: float) -> float:
    return round(float(value) + 1e-12, 6)


def compute_employer_levies(
    gross_monthly_eur: float,
    profile: dict,
    parameters: dict
) -> dict:
    employer_levies = parameters["social_security"]["employer_levies"]

    contribution_base = min(
        gross_monthly_eur,
        employer_levies["max_premieloon_monthly_eur"]
    )

    ww_monthly = contribution_base * profile["ww_rate"]
    aof_monthly = contribution_base * profile["aof_rate"]
    zvw_monthly = contribution_base * profile["zvw_rate"]
    wko_monthly = contribution_base * profile["wko_rate"]
    whk_monthly = contribution_base * profile["whk_rate"]

    employer_contributions_monthly = (
        ww_monthly
        + aof_monthly
        + zvw_monthly
        + wko_monthly
        + whk_monthly
    )

    return {
        "contribution_base_monthly_eur": contribution_base,
        "ww_monthly_eur": ww_monthly,
        "aof_monthly_eur": aof_monthly,
        "zvw_monthly_eur": zvw_monthly,
        "wko_monthly_eur": wko_monthly,
        "whk_monthly_eur": whk_monthly,
        "employer_contributions_monthly_eur": employer_contributions_monthly
    }


def compute_row(
    profile: dict,
    smic_multiple: float,
    parameters: dict
) -> dict:
    wage_reference = parameters["wage_reference"]["gross_monthly_eur"]

    gross_monthly_eur = (
        wage_reference
        * smic_multiple
    )

    # The Netherlands has no separate visible employee social-security
    # contribution: AOW/Anw/Wlz national-insurance premiums are bundled
    # into the Box 1 loonheffing computed below. Nothing is deducted from
    # gross pay before income tax.
    net_before_income_tax = gross_monthly_eur

    levies = compute_employer_levies(
        gross_monthly_eur=gross_monthly_eur,
        profile=profile,
        parameters=parameters
    )

    employer_contributions = levies["employer_contributions_monthly_eur"]

    tax_result = compute_loonheffing_2026(gross_monthly_eur)

    loonheffing_monthly_eur = tax_result["loonheffing_monthly_eur"]

    net_after_loonheffing = (
        net_before_income_tax
        - loonheffing_monthly_eur
    )

    employer_cost = (
        gross_monthly_eur
        + employer_contributions
    )

    social_wedge_before_income_tax = (
        employer_cost
        - net_before_income_tax
    )

    total_wedge_after_loonheffing = (
        employer_cost
        - net_after_loonheffing
    )

    return {
        "country": parameters["country"],
        "country_code": parameters["country_code"],
        "version": parameters["version"],
        "effective_from": parameters["effective_from"],

        "profile_id": profile["profile_id"],
        "profile_label_fr": profile["label_fr"],
        "profile_label_en": profile["label_en"],

        "smic_multiple": smic_multiple,
        "gross_monthly_eur": round_money(gross_monthly_eur),

        "employee_contribution_rate": 0.0,
        "employee_contributions_monthly_eur": 0.0,

        "ww_rate": profile["ww_rate"],
        "aof_rate": profile["aof_rate"],
        "zvw_rate": profile["zvw_rate"],
        "wko_rate": profile["wko_rate"],
        "whk_rate": profile["whk_rate"],

        "contribution_base_monthly_eur": round_money(
            levies["contribution_base_monthly_eur"]
        ),
        "ww_monthly_eur": round_money(levies["ww_monthly_eur"]),
        "aof_monthly_eur": round_money(levies["aof_monthly_eur"]),
        "zvw_monthly_eur": round_money(levies["zvw_monthly_eur"]),
        "wko_monthly_eur": round_money(levies["wko_monthly_eur"]),
        "whk_monthly_eur": round_money(levies["whk_monthly_eur"]),

        "employer_contributions_monthly_eur": round_money(
            employer_contributions
        ),

        "employer_contribution_rate": (
            employer_contributions
            / gross_monthly_eur
            if gross_monthly_eur > 0
            else np.nan
        ),

        "net_before_income_tax_monthly_eur": round_money(
            net_before_income_tax
        ),

        "annual_income_eur": tax_result["annual_income_eur"],
        "annual_box1_tax_eur": tax_result["annual_box1_tax_eur"],
        "annual_algemene_heffingskorting_eur": tax_result[
            "annual_algemene_heffingskorting_eur"
        ],
        "annual_arbeidskorting_eur": tax_result["annual_arbeidskorting_eur"],
        "annual_loonheffing_eur": tax_result["annual_loonheffing_eur"],

        "loonheffing_monthly_eur": round_money(loonheffing_monthly_eur),

        "net_after_loonheffing_monthly_eur": round_money(
            net_after_loonheffing
        ),

        "employer_cost_monthly_eur": round_money(
            employer_cost
        ),

        "social_wedge_monthly_eur": round_money(
            social_wedge_before_income_tax
        ),

        "total_wedge_after_loonheffing_monthly_eur": round_money(
            total_wedge_after_loonheffing
        ),

        "cost_to_net_ratio": (
            employer_cost
            / net_before_income_tax
            if net_before_income_tax > 0
            else np.nan
        ),

        "cost_to_net_after_loonheffing_ratio": (
            employer_cost
            / net_after_loonheffing
            if net_after_loonheffing > 0
            else np.nan
        )
    }


def build_dataset(parameters: dict) -> pd.DataFrame:
    grid_parameters = parameters["grid"]

    wage_grid = np.round(
        np.arange(
            grid_parameters["min_multiple"],
            grid_parameters["max_multiple"] + 0.001,
            grid_parameters["step"]
        ),
        2
    )

    rows = []

    for profile in parameters["profiles"]:
        for smic_multiple in wage_grid:
            rows.append(
                compute_row(
                    profile=profile,
                    smic_multiple=float(smic_multiple),
                    parameters=parameters
                )
            )

    dataset = pd.DataFrame(rows)

    return dataset


def add_marginal_indicators(dataset: pd.DataFrame) -> pd.DataFrame:
    dataset = dataset.sort_values(
        [
            "profile_id",
            "smic_multiple"
        ]
    ).copy()

    dataset["delta_gross_monthly_eur"] = dataset.groupby(
        "profile_id"
    )["gross_monthly_eur"].diff()

    dataset["delta_net_before_income_tax_monthly_eur"] = dataset.groupby(
        "profile_id"
    )["net_before_income_tax_monthly_eur"].diff()

    dataset["delta_net_after_loonheffing_monthly_eur"] = dataset.groupby(
        "profile_id"
    )["net_after_loonheffing_monthly_eur"].diff()

    dataset["delta_employer_cost_monthly_eur"] = dataset.groupby(
        "profile_id"
    )["employer_cost_monthly_eur"].diff()

    dataset["delta_social_wedge_monthly_eur"] = dataset.groupby(
        "profile_id"
    )["social_wedge_monthly_eur"].diff()

    dataset["delta_total_wedge_after_loonheffing_monthly_eur"] = dataset.groupby(
        "profile_id"
    )["total_wedge_after_loonheffing_monthly_eur"].diff()

    dataset["marginal_net_before_income_tax_rate"] = (
        dataset["delta_net_before_income_tax_monthly_eur"]
        / dataset["delta_gross_monthly_eur"]
    )

    dataset["marginal_net_after_loonheffing_rate"] = (
        dataset["delta_net_after_loonheffing_monthly_eur"]
        / dataset["delta_gross_monthly_eur"]
    )

    dataset["marginal_employer_cost_rate"] = (
        dataset["delta_employer_cost_monthly_eur"]
        / dataset["delta_gross_monthly_eur"]
    )

    dataset["marginal_social_wedge_rate"] = (
        dataset["delta_social_wedge_monthly_eur"]
        / dataset["delta_gross_monthly_eur"]
    )

    dataset["marginal_total_wedge_after_loonheffing_rate"] = (
        dataset["delta_total_wedge_after_loonheffing_monthly_eur"]
        / dataset["delta_gross_monthly_eur"]
    )

    return dataset


def run_quality_checks(dataset: pd.DataFrame) -> dict:
    net_identity_error = (
        dataset["gross_monthly_eur"]
        - dataset["employee_contributions_monthly_eur"]
        - dataset["net_before_income_tax_monthly_eur"]
    ).abs().max()

    loonheffing_identity_error = (
        dataset["net_before_income_tax_monthly_eur"]
        - dataset["loonheffing_monthly_eur"]
        - dataset["net_after_loonheffing_monthly_eur"]
    ).abs().max()

    employer_cost_identity_error = (
        dataset["gross_monthly_eur"]
        + dataset["employer_contributions_monthly_eur"]
        - dataset["employer_cost_monthly_eur"]
    ).abs().max()

    social_wedge_identity_error = (
        dataset["employer_cost_monthly_eur"]
        - dataset["net_before_income_tax_monthly_eur"]
        - dataset["social_wedge_monthly_eur"]
    ).abs().max()

    total_wedge_after_tax_identity_error = (
        dataset["employer_cost_monthly_eur"]
        - dataset["net_after_loonheffing_monthly_eur"]
        - dataset["total_wedge_after_loonheffing_monthly_eur"]
    ).abs().max()

    return {
        "rows": len(dataset),
        "profiles": dataset["profile_id"].nunique(),
        "max_net_identity_error": net_identity_error,
        "max_loonheffing_identity_error": loonheffing_identity_error,
        "max_employer_cost_identity_error": employer_cost_identity_error,
        "max_social_wedge_identity_error": social_wedge_identity_error,
        "max_total_wedge_after_tax_identity_error": total_wedge_after_tax_identity_error
    }


def main() -> None:
    parameters = load_parameters()

    dataset = build_dataset(parameters)
    dataset = add_marginal_indicators(dataset)

    OUTPUT_PATH.parent.mkdir(
        parents=True,
        exist_ok=True
    )

    dataset.to_csv(
        OUTPUT_PATH,
        index=False,
        encoding="utf-8"
    )

    checks = run_quality_checks(dataset)

    print("Netherlands dataset created.")
    print(f"Output: {OUTPUT_PATH}")
    print()
    print("Quality checks")
    print(f"Profiles: {checks['profiles']}")
    print(f"Rows: {checks['rows']}")

    print(
        "Max net identity error: "
        f"{checks['max_net_identity_error']:.10f}"
    )

    print(
        "Max loonheffing identity error: "
        f"{checks['max_loonheffing_identity_error']:.10f}"
    )

    print(
        "Max employer cost identity error: "
        f"{checks['max_employer_cost_identity_error']:.10f}"
    )

    print(
        "Max social wedge identity error: "
        f"{checks['max_social_wedge_identity_error']:.10f}"
    )

    print(
        "Max total wedge after tax identity error: "
        f"{checks['max_total_wedge_after_tax_identity_error']:.10f}"
    )

    print()
    print("Sample")
    print(
        dataset[
            [
                "smic_multiple",
                "gross_monthly_eur",
                "net_before_income_tax_monthly_eur",
                "loonheffing_monthly_eur",
                "net_after_loonheffing_monthly_eur",
                "employer_cost_monthly_eur",
                "cost_to_net_after_loonheffing_ratio"
            ]
        ]
        .query("smic_multiple in [1.0, 1.5, 2.0, 3.0, 6.0]")
        .to_string(index=False)
    )


if __name__ == "__main__":
    main()
